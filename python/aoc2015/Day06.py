...
def parse_command(line):
  ...
  tr_index = 3
  if command == 'turn':
    command = command + ' ' + words[1]
    bl_index = 2
    tr_index = 4

  x1 = int(words[bl_index].split(',')[0])
  y1 = int(words[bl_index].split(',')[1])
  x2 = int(words[tr_index].split(',')[0])
  y2 = int(words[tr_index].split(',')[1])

  return command, x1, y1, x2, y2


def apply_instructions(grid, include_brightness = False):
  for line in input.split('\n'):
    command, x1, y1, x2, y2 = parse_command(line)
    for i in range(x1, x2+1):
      for j in range(y1, y2+1):
        if command == "turn on":
          grid[i][j] = grid[i][j] + 1 if include_brightness else 1
        elif command == "turn off":
          new_val = grid[i][j] - 1 if include_brightness else 0
          grid[i][j] = 0 if new_val < 0 else new_val
        else:
          val_if_brightness_is_false = 1 if grid[i][j] == 0 else 0
          grid[i][j] = grid[i][j] + 2 if include_brightness else val_if_brightness_is_false
