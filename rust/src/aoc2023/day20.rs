...
fn part2(input_data: &String) -> String {
    let modules = parse_modules(input_data);
    let r = count_button_presses(&modules);

    println!("count_button_presses: {:?}", r);
    (r.0 * r.1).to_string()
}
...
// Actual solution
// --------------------------------------------------------------------------------------
fn count_button_presses(modules: &Modules) -> (i64, i64, i64) {
    let mut count_button_presses = 0;
    ...
    loop {
        ...
        while !queue.is_empty() {
            ...
            if current_pulse == Pulse::Low && key == "rx" {
                // if count_button_presses == 1001 {
                return (_count_low, _count_high, count_button_presses);
            }
