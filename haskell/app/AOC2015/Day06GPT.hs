...
data Instruction = TurnOn Coord Coord | TurnOff Coord Coord | Toggle Coord Coord
...
parseInstruction str =
  let instructionParts = words str
      splitStr = splitOn ","
      getCoordValue i = (read . (!! i) . splitStr)
      parseCoord s = Coord (getCoordValue 0 s) (getCoordValue 1 s)
   in case instructionParts of
        ["toggle", bl, "through", tr] -> Toggle (parseCoord bl) (parseCoord tr)
        ["turn", "on", bl, "through", tr] -> TurnOn (parseCoord bl) (parseCoord tr)
        ["turn", "off", bl, "through", tr] -> TurnOff (parseCoord bl) (parseCoord tr)
        _ -> error "Invalid instruction"

-- Part 1: Binary grid (on/off) using Unboxed Arrays
solvePart1 :: [Instruction] -> Int
solvePart1 instructions = runST $ do
  grid <- newArray ((0, 0), (999, 999)) False :: ST s (STUArray s (Int, Int) Bool)
  processInstructionsParallel grid instructions
  countLightsOn grid

-- Parallel processing of instructions
processInstructionsParallel :: STUArray s (Int, Int) Bool -> [Instruction] -> ST s ()
processInstructionsParallel grid instructions = do
  let chunkSize = length instructions `div` 16 -- Split work into chunks (adjust chunk size as needed)
  let chunks = splitIntoChunks chunkSize instructions
  -- Process each chunk in parallel
  let actions = map (processInstructionChunk grid) chunks
  sequence_ $ using actions $ parListChunk chunkSize rseq

processInstructionChunk :: STUArray s (Int, Int) Bool -> [Instruction] -> ST s ()
processInstructionChunk grid = mapM_ (processInstruction grid)

processInstruction :: STUArray s (Int, Int) Bool -> Instruction -> ST s ()
processInstruction grid (TurnOn (Coord blx bly) (Coord trx try)) =
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> writeArray grid (x, y) True
processInstruction grid (TurnOff (Coord blx bly) (Coord trx try)) =
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> writeArray grid (x, y) False
processInstruction grid (Toggle (Coord blx bly) (Coord trx try)) =
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> do
      currentVal <- readArray grid (x, y)
      writeArray grid (x, y) (not currentVal)

-- Count how many lights are on
countLightsOn :: STUArray s (Int, Int) Bool -> ST s Int
countLightsOn grid = do
  elements <- getAssocs grid
  return $ length $ filter snd elements

-- Part 2: Grid with brightness levels
solvePart2 :: [Instruction] -> Int
solvePart2 instructions = runST $ do
  grid <- newArray ((0, 0), (999, 999)) 0 :: ST s (STUArray s (Int, Int) Int)
  processInstructionsParallelBrightness grid instructions
  countTotalBrightness grid

processInstructionsParallelBrightness :: STUArray s (Int, Int) Int -> [Instruction] -> ST s ()
processInstructionsParallelBrightness grid instructions = do
  let chunkSize = length instructions `div` 16 -- Adjust chunk size
  let chunks = splitIntoChunks chunkSize instructions
  let actions = map (processInstructionChunkBrightness grid) chunks
  sequence_ $ using actions $ parListChunk chunkSize rseq

processInstructionChunkBrightness :: STUArray s (Int, Int) Int -> [Instruction] -> ST s ()
processInstructionChunkBrightness grid = mapM_ (processInstructionBrightness grid)

processInstructionBrightness :: STUArray s (Int, Int) Int -> Instruction -> ST s ()
processInstructionBrightness grid (TurnOn (Coord blx bly) (Coord trx try)) =
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> do
      currentVal <- readArray grid (x, y)
      writeArray grid (x, y) (currentVal + 1)
processInstructionBrightness grid (TurnOff (Coord blx bly) (Coord trx try)) =
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> do
      currentVal <- readArray grid (x, y)
      writeArray grid (x, y) (max 0 (currentVal - 1))
processInstructionBrightness grid (Toggle (Coord blx bly) (Coord trx try)) =
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> do
      currentVal <- readArray grid (x, y)
      writeArray grid (x, y) (currentVal + 2)

-- Count total brightness
countTotalBrightness :: STUArray s (Int, Int) Int -> ST s Int
countTotalBrightness grid = do
  elements <- getAssocs grid
...
-- Example usage (with your parsed input):
main :: IO ()
main = do
  -- Assume `input` is a list of instructions
  let instructions = map parseInstruction (lines input)
  print $ solvePart1 instructions  -- Part 1 solution
  print $ solvePart2 instructions  -- Part 2 solution
