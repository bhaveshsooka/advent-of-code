...
printAoC2015Day06GPTAnswer :: IO ()
printAoC2015Day06GPTAnswer = do
  putStrLn "------ Day 06 GPT ------"
...
parseInstruction str =
  let instructionParts = T.words str
      splitStr = T.splitOn ","
      getCoordValue i = (read . T.unpack . (!! i) . splitStr)
...
instructions = parseInstruction <$> T.lines input

-- Part 1: Binary grid (on/off) using Unboxed Arrays
part1 :: Int
part1 = runST $ do
  grid <- newArray ((0, 0), (999, 999)) False :: ST s (STUArray s (Int, Int) Bool)
  processInstructionsParallel grid
  countLightsOn grid

-- Parallel processing of instructions
processInstructionsParallel :: STUArray s (Int, Int) Bool -> ST s ()
processInstructionsParallel grid = do
...
part2 = runST $ do
  ...
  processInstructionsParallelBrightness grid
  countTotalBrightness grid

processInstructionsParallelBrightness :: STUArray s (Int, Int) Int -> ST s ()
processInstructionsParallelBrightness grid = do
