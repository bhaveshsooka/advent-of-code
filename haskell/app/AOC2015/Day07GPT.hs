...
data Wire = WInt Word16 | WStr String deriving (Show, Eq)
...
type Cache = Map String Word16

part1 :: Word16
part1 = fromJust $ lookup "a" $ processSignals signals mempty

part2 :: Word16
part2 = fromJust $ lookup "a" $ processSignals signals (insert "b" part1 mempty)

processSignals :: Map String Op -> Cache -> Cache
processSignals m cache = foldl (processSignal m) cache (keys m)

processSignal :: Map String Op -> Cache -> String -> Cache
