...
applyInstruction grid brightness instruction = do
  let (Coord blx bly, Coord trx try) = getArrayBounds instruction
  forM_ [blx .. trx] $ \x ->
    forM_ [bly .. try] $ \y -> do
...
getArrayBounds (TurnOn bl tr) = (bl, tr)
getArrayBounds (TurnOff bl tr) = (bl, tr)
getArrayBounds (Toggle bl tr) = (bl, tr)

updateValue :: Bool -> Int -> Instruction -> Int
updateValue brightness val (TurnOn _ _) = if brightness then val + 1 else 1
updateValue brightness val (TurnOff _ _) = if brightness then max 0 (val - 1) else 0
updateValue brightness val (Toggle _ _) = if brightness then val + 2 else toggle val
