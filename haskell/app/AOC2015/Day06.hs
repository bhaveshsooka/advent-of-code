...
part1Maps =
  let instructions = parseInstruction <$> lines input
   in countLightsMap $ foldl' processInstructionMap (emptyMap 999 999) instructions

testMaps :: Map Coord Int
testMaps =
  let instructions = [testInstruction1, testInstruction2, testInstruction3]
   in foldl (\acc instruction -> processInstructionMap acc instruction) (emptyMap 4 4) instructions

countLightsMap :: Map Coord Int -> Int
countLightsMap grid = sum $ filter (== 1) $ (elems grid)

emptyMap :: Int -> Int -> Map Coord Int
emptyMap a b =
  let coords = [(Coord x y, 0) | x <- [0 .. a], y <- [0 .. b]]
   in fromList coords

processInstructionMap :: Map Coord Int -> Instruction -> Map Coord Int
processInstructionMap grid instruction =
  let newCoords x1 y1 x2 y2 = [Coord x y | x <- [x1 .. x2], y <- [y1 .. y2]]
   in case instruction of
        TurnOn (Coord blx bly) (Coord trx try) -> updateMap grid (newCoords blx bly trx try) (const 1)
        TurnOff (Coord blx bly) (Coord trx try) -> updateMap grid (newCoords blx bly trx try) (const 0)
        Toggle (Coord blx bly) (Coord trx try) -> updateMap grid (newCoords blx bly trx try) toggle
